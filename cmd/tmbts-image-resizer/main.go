...
import (
	...
	"github.com/kyleshepherd/tmbts-image-resizer/internal/config"
	"github.com/kyleshepherd/tmbts-image-resizer/internal/version"
)
...
func tmbtsimageresizerCmd() *cobra.Command {
	var configPath string
	cmd := &cobra.Command{
		Use:           "tmbts-image-resizer",
		...
		RunE: tmbtsimageresizerRun,
	}
	// Global flags
	pflags := cmd.PersistentFlags()
	pflags.StringVarP(&configPath, "config", "c", "", "path to configuration file (default is $HOME/.config/tmbts-image-resizer.toml)")
...
// tmbts-image-resizerRun is executed when the CLI executes
// the tmbts-image-resizer command
func tmbtsimageresizerRun(cmd *cobra.Command, _ []string) error {
	return cmd.Help()
}
